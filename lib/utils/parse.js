...
console.log('parser');
function parseInvalidJSON(obj) {
    let newObj = {};
    obj.replace(/'/gm, '"').replace(/{/gm, '{ ').split(':').forEach((word, i, arr) => {
        ...
        if (word.trim().split('')[0] !== '"' && word.trim().split('')[0] !== '{') {
            ...
            console.log(splitWord);
            if (word.split('[')[1])
                splitWord[0] = word.replace('[' + arrWord + ']', '["' + arrWord + '"]');
        ...
        if (i === obj.replace(/'/gm, '"').replace(/{/gm, '{ ').split(':').length - 1)
            newObj = JSON.parse(arr.join(':'));
        // return arr.join(':')
    });
    return newObj;
}
class GoatParse {
    constructor(input) {
        ...
        if (typeof input !== 'string')
            console.log(this.parseError('This package can only work with strings at the moment.'));
    ...
    parseError(message = 'A parsing error has occurred.') {
